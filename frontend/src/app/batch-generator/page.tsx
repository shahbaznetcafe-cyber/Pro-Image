...
export default function BatchGeneratorPage() {
  ...
  const visiblePresets = useMemo(
    () => PRESETS.filter((preset) => region === "All regions" || preset.regions.includes(region) || preset.regions.includes("Global")),
    [region],
  );
    ...
    <main className="min-h-screen bg-[#f6f8f5] text-[#172018]">
      <div className="mx-auto flex min-h-screen w-full max-w-7xl flex-col px-5 py-5 sm:px-8 lg:px-10">
        <header className="flex flex-wrap items-center justify-between gap-4 border-b border-[#dce4d8] pb-5">
          <div>
            <Link href="/" className="text-sm font-semibold uppercase tracking-[0.18em] text-[#4f6f4f] hover:text-[#1f4f2a]">
              SBZ SellImage Pro
            </Link>
          ...
          <div className="flex flex-col gap-5">
            <div className="rounded-lg border border-[#dce4d8] bg-white p-5 shadow-sm">
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                ...
                <label className="ml-auto flex items-center gap-2 text-sm font-medium text-[#405040]">
                  Region
                  <select
                    value={region}
                    onChange={(event) => setRegion(event.target.value)}
                    className="rounded-md border border-[#c8d7c5] bg-white px-3 py-2 outline-none focus:ring-2 focus:ring-[#537c55]"
                  >
                ...
                {visiblePresets.map((preset) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {preset.confidence && preset.confidence !== "verified" ? (
                        <p className="mt-3 text-xs font-medium text-[#8a6412]">
                          {preset.confidence === "provisional" ? "Verify category rules" : "Regional rules apply"}
                        </p>
                      ) : null}
